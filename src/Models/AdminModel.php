...
class AdminModel
{
    ...
    // funtion insert logo
    public function insertLogo($site_name) {
        ...
        if (in_array($filecheck, $file_ext_stored)) {
            ...
            $img = Image::make($file_tmp)->resize(50, 50)->save($resized_loc);


            move_uploaded_file($img, $resized_loc);

        }
        $query = $this->conn->prepare("INSERT INTO config(logo, siteName) VALUES(?, ?)");
        $query->bind_param("ss",   $location, $site_name);
        $query->execute();
        $ans = $query->get_result();
        return $ans;
    }
